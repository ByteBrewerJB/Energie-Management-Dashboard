...
def create_solar_panel(
    *,
    ...
    Enforces a single-installation policy.
    """
    existing_panels = crud_solar_panel.get_multi(db)
    if existing_panels:
        raise HTTPException(
            status_code=400,
            detail="A solar panel installation already exists. Use the PUT endpoint to update it.",
        )
